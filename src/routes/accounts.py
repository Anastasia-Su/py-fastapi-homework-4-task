...
async def register_user(
    ...
    try:
        ...
        activation_link = (
            f"http://127.0.0.1:8000/api/v1/accounts/activate-link/"
...
async def activate_account(
    ...
    background_tasks.add_task(
        ...
        str(user.email),
        login_link="http://127.0.0.1:8000/docs#/accounts/login_user_api_v1_accounts_login__post",  # example login link
    )
...
async def request_password_reset_token(
    ...
    background_tasks.add_task(
        ...
        str(user.email),
        reset_link="http://127.0.0.1:8000/docs#/accounts/reset_password_api_v1_accounts_reset_password_complete__post",
    )
...
async def reset_password(
    ...
    try:
        ...
        background_tasks.add_task(
            ...
            str(user.email),
            login_link="http://127.0.0.1:8000/docs#/accounts/login_user_api_v1_accounts_login__post",
        )
...
@router.get(
    ...
    description="Activate a user's account by clicking a link from their email.",
    response_class=HTMLResponse,
    responses={
        400: {
            "description": "Bad Request - The activation token is invalid, expired, or the account is already active.",
            "content": {
                "text/html": {
                    "example": "<h1>Invalid or expired activation token.</h1>"
                }
            },
        },
        500: {
            ...
            "content": {
                "text/html": {"example": "<h1>An unexpected error occurred.</h1>"}
            },
        },
    },
)
async def activate_account_link(
    email: str,
    token: str,
    background_tasks: BackgroundTasks,
    db: AsyncSession = Depends(get_db),
    ...
    """
    Endpoint to activate a user's account when they click the activation link in their email.
    """
    try:
        activation_data = UserActivationRequestSchema(email=email, token=token)
        ...
        return HTMLResponse("<h1>Account activated successfully!</h1>")

    except HTTPException as http_exc:
        return HTMLResponse(
            f"<h1>{http_exc.detail}</h1>", status_code=http_exc.status_code
        )

    except Exception as exc:
        print(f"Unexpected error during account activation: {exc}")
